...
//				dp[beg][beg + len] = (ch[beg] == ch[beg + len] && (len < 3 ? true : dp[beg + 1][beg + len - 1]));
		...
		//return computeRes(ch, dp, 0, map);

        return computeRes2(ch, dp);
    }

    //用map记忆化搜索 自顶向下 用于递归中
    public int computeRes(char[] ch, boolean[][] dp, int beg, Map<Integer, Integer> map) {
        if (beg == ch.length)
            return -1;
        ...
        for (int i = beg; i < ch.length; i++) {
            if (dp[beg][i]) {
                len = Integer.min(len, computeRes(ch, dp, i + 1, map) + 1);            //永远不会出现返回MAX_VALUE的情况，因为最差情况都可以分成一个一个的
            }
        }
        map.put(beg, len);
        return len;
    ...
    //用数组动态规划 自底向上
    public int computeRes2(char[] ch, boolean[][] dp) {
